...
export function NextButton() {
  const nextComic = async () => {
    await fetch("http://calvinpi.home:8000/api/comics/next", {
      method: "POST",
    });
  };
  return (
...
export function BackButton() {
  const previousComic = async () => {
    await fetch("http://calvinpi.home:8000/api/comics/previous", {
      method: "POST",
    });
  };
  return (
...
export function TodayButton() {
  const todaysComic = async () => {
    await fetch("http://calvinpi.home:8000/api/comics/today", {
      method: "POST",
    });
  };
  return (
